...
def is_session_generation_active(generation: int) -> bool:
...
def save_speech(
    ...
    *,
    ...
    if session_generation is not None and not is_session_generation_active(session_generation):
        return None
...
def set_meeting_goal(goal: str, *, session_generation: int = None) -> bool:
    if session_generation is not None and not is_session_generation_active(session_generation):
        return False
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
...
def clear_session_data() -> int:
    """現在の会議セッションのデータをすべて削除し、新しい世代番号を返す"""
